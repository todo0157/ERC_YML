...
# 1) 설정 (ABS용)
# =========================================================
# v1.0 전처리 코드로 생성된 특징 파일을 사용합니다.
INPUT_XLSX = r"ERC/PM/features_pm_rich_p10to100_corr.xlsx"
SHEET_NAME = "features" 
...
OUT_DIR = "ABS_Ra_p10to100_results_corr"
OUT_XLSX = "results_ABS_p10to100_compare_corr.xlsx"
...
def select_only_pct_columns(X_all: pd.DataFrame, pct: int) -> pd.DataFrame:
    """
    일반 특징(_pXX_)과 상관관계 특징(CC_pXX_)을 모두 선택합니다.
    """
    key1 = f"_p{pct}_"
    key2 = f"CC_p{pct}_"
    cols = []
...
def plot_compare_lines(pcts, y_full, y_top, baseline, ylabel, title, outpath, label_top="TopK"):
    ...
    plt.plot(pcts, y_full, marker="o", color="black", label="Full Features (Rich)")
...
def main():
    ...
    # 1) 특징 데이터 로드
    print(f"Loading Feature Data: {INPUT_XLSX}")
    try:
    ...
    try:
        ...
        # 병합을 위해 컬럼명 정리
        # df_feat의 'sample_id'와 df_label의 'Unnamed: 0' (또는 첫 번째 컬럼) 매칭
        id_col_label = df_label.columns[0] # 'Unnamed: 0'
        
        # 병합
        df = pd.merge(df_feat, df_label[[id_col_label, LABEL_ORIGIN_COL]], 
                      left_on="sample_id", right_on=id_col_label, how="inner")
        
        if df.empty:
            print("[Error] Merge result is empty.")
            return
    ...
    df = df.dropna(subset=[actual_label]).copy()
    ...
    # 학습에 사용하지 않을 컬럼들 제거 (라벨, ID, 원본 라벨 등)
    drop_cols = [actual_label, "sample_id", "n_points", id_col_label, LABEL_ORIGIN_COL]
    ...
    for pct in PCTS:
        ...
        line_full_rmse.append(r_full); line_full_re.append(re_full)
        prediction_df[f"Ra_pred_Full_p{pct}"] = yhat_full

        # 2) Importance
        MODEL.fit(Xp, y)
        perm = permutation_importance(MODEL, Xp, y, n_repeats=PERM_REPEATS, random_state=RANDOM_STATE, scoring="neg_mean_squared_error")
        ...
        save_importance_bar(Xp.columns, perm_scores, f"ABS Importance - p{pct}%", os.path.join(imp_dir, f"importance_p{pct}.png"))
        ...
        line_focus_rmse.append(r_focus); line_focus_re.append(re_focus)
    ...
    plot_compare_lines(valid_pcts, line_full_rmse, line_focus_rmse, base_rmse, "RMSE (um)", "ABS Ra Prediction - RMSE", os.path.join(fig_dir, "RMSE_Comparison.png"))
    plot_compare_lines(valid_pcts, line_full_re, line_focus_re, base_re, "RE (%)", "ABS Ra Prediction - RE (%)", os.path.join(fig_dir, "RE_Comparison.png"))
