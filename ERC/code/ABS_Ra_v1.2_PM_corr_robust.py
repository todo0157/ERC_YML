...
# =========================================================
# ABS Ra prediction (robust runner)
# - 기존 ABS_Ra_v1.0_PM.py는 유지
# - corr_robust 특징 파일을 입력으로 사용
# - 추가로 feature matrix winsorize(열별 분위수 클리핑)로 이상치에 더 강하게
# =========================================================

# 입력 특징 파일(robust)
INPUT_XLSX = r"ERC/PM/features_pm_rich_p10to100_corr_robust.xlsx"
SHEET_NAME = "features"
...
# 출력(기존 결과와 분리)
OUT_DIR = "ABS_Ra_p10to100_results_corr_robust"
OUT_XLSX = "results_ABS_p10to100_compare_corr_robust.xlsx"

# robust 옵션: 열별 winsorize
WINSOR_Q_LOW = 0.01
WINSOR_Q_HIGH = 0.99
...
def clean_feature_matrix(X: pd.DataFrame) -> pd.DataFrame:
    ...
    2) NaN 50% 초과 컬럼 제거
    3) 결측 median 채움
    4) 열별 winsorize(1~99%)로 outlier 영향 완화
    5) 상수 컬럼 제거
    """
...
def save_importance_bar(
    cols, scores, title, outpath, topn=25, xlabel="Permutation Importance (MSE Increase)"
):
    scores = np.asarray(scores, dtype=float)
    order = np.argsort(scores)[::-1]
...
def plot_compare_lines(
    pcts, y_full, y_top, baseline, ylabel, title, outpath, label_top="TopK"
):
...
def main():
    ...
    for pct in PCTS:
        ...
        save_importance_bar(
            Xp.columns, perm_scores, f"ABS Importance (robust) - p{pct}%",
